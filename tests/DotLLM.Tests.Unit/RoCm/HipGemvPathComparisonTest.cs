...
/// </summary>
[Trait("Category", "GPU")]
public class HipGemvPathComparisonTest
{
    private readonly ITestOutputHelper _out;
    public HipGemvPathComparisonTest(ITestOutputHelper output) => _out = output;
    ...
    private static bool CudaAvailableProbe() => HipDevice.IsAvailable();

    [SkippableFact]
    public void CompareGemvPaths_RealModelWeights_Layer0_QProjection()
    {
        Skip.IfNot(IsCudaDriverPresent(), "No ROCm GPU available");
    ...
    private unsafe void CompareGemvPaths_RealModelWeights_Layer0_QProjection_Impl()
    {
        ...
        float[] pathB;
        {
            ...
            Skip.If(hsacoDir == null, "Hsaco files not found");
            using var kernels = new HipKernels(hsacoDir!);
            ...
            HipApi.hipMemcpyHtoD(devW, lw.QWeight, (nuint)wBytes).ThrowOnError();
            fixed (float* pX = x)
                HipApi.hipMemcpyHtoD(devX, (nint)pX, (nuint)xBytes).ThrowOnError();
            ...
            fixed (float* pY = pathB)
                HipApi.hipMemcpyDtoH((nint)pY, devY, (nuint)yBytes).ThrowOnError();
    ...
    private unsafe void TestProjection(string name, byte* weightPtr, QuantizationType qt,
                                        int n, int k, float[] x, HipKernels kernels, HipStream stream)
    {
    ...
    private unsafe float[] RunGpuGemv(nint weightHost, float[] x, int n, int k,
                                       HipKernels kernels, HipStream stream)
    {
        nint s = stream.Handle;
        long wBytes = Dequantize.RowByteSize(k, QuantizationType.Q8_0) * n;
        long xBytes = (long)k * sizeof(float);
        ...
        try
        {
            HipApi.hipMemcpyHtoD(devW, weightHost, (nuint)wBytes).ThrowOnError();
            fixed (float* pX = x)
                HipApi.hipMemcpyHtoD(devX, (nint)pX, (nuint)xBytes).ThrowOnError();
